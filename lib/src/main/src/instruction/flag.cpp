...
namespace m6502
{
    ...
    //
    // These instructions are implied mode, have a length of one byte and require two
    // machine cycles.
    //
    // MNEMONIC                       HEX
    // CLC (CLear Carry)              $18
    ...
    //   it to its prior status on completion of the interrupt service routine. If you
    //   want your interrupt service routine to permit other maskable interrupts, you must
    //   clear the I flag in your code.
    //
    //   The Decimal flag controls how the 6502 adds and subtracts. If set, arithmetic is
    //   carried out in packed binary coded decimal. This flag is unchanged by interrupts
    //   and is unknown on power-up. The implication is that a CLD should be included in
    //   boot or interrupt coding.
    //
    //   The Overflow flag is generally misunderstood and therefore under-utilised.
    //   After an ADC or SBC instruction, the overflow flag will be set if the
    //   twos complement result is less than -128 or greater than +127, and it will
    //   cleared otherwise. In twos complement, $80 through $FF represents -128 through -1,
    ...
    //   LDA #$81 ;   -127
    //   ADC #$FF ; +   -1
    // the overflow flag is 0 (-127 + -1 = -128). The overflow flag is not affected by
    // increments, decrements, shifts and logical operations
    // i.e. only ADC, BIT, CLV, PLP, RTI and SBC affect it. There is no op code to set
    // the overflow but a BIT test on an RTS instruction will do the trick.
    //----------------------------------------
